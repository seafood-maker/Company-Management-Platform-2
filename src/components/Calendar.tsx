...
const CalendarView: React.FC<CalendarProps> = ({ schedules, onEdit, onDelete, currentUser, users }) => {
  ...
  const [selectedDayInfo, setSelectedDayInfo] = useState<{day: number, schedules: Schedule[]} | null>(null); // 月曆模式的彈窗狀態
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col h-full overflow-hidden relative">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl">
            <button 
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${viewMode === 'list' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}
            >
              <i className="fas fa-list-ul mr-1.5"></i>詳情列表
            </button>
            <button 
              onClick={() => setViewMode('grid')}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${viewMode === 'grid' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}
            >
                ...
                <div key={day} className={`flex flex-col md:flex-row p-3 transition ${activeToday ? 'bg-indigo-50/30' : ''}`}>
                  ...
                  <div className="flex-1 space-y-2">
                    {daySchedules.length > 0 ? daySchedules.map(s => {
                      ...
                      return (
                        <div key={s.id} onClick={() => setExpandedId(isExpanded ? null : s.id)} className={`group border rounded-xl transition-all bg-white overflow-hidden ${isExpanded ? 'border-indigo-300 shadow-md' : 'border-slate-100 hover:border-indigo-200'}`}>
                          <div className="p-3 flex items-center justify-between gap-2 text-[13px]">
                            <div className="flex items-center space-x-3 flex-1 min-w-0">
                              ...
                              <span className="hidden sm:inline-block px-1.5 py-0.5 bg-slate-100 rounded text-[10px] font-bold">{s.category}</span>
                          ...
                          {isExpanded && (
                            <div className="px-3 pb-4 pt-1 border-t border-slate-50 animate-in slide-in-from-top-1 text-[12px] space-y-2">
                              <div className="flex flex-col"><span className="text-slate-400 font-bold mb-1">事由/目的地</span><div className="bg-slate-50 p-2 rounded-lg italic">{s.purpose || '無填寫事由'}</div></div>
                              {s.accompanimentIds?.length > 0 && <div className="flex flex-col"><span className="text-slate-400 font-bold mb-1">同行同仁</span><div className="flex flex-wrap gap-1">{getCompanionNames(s.accompanimentIds).split(',').map((n, i) => <span key={i} className="bg-pink-50 text-pink-600 px-2 py-0.5 rounded-full font-bold text-[10px]">{n.trim()}</span>)}</div></div>}
                              {s.vehicleId && <div className="flex justify-between md:hidden"><span className="text-slate-400 font-bold">預約車輛</span><span className="font-mono text-amber-700 bg-amber-50 px-2 rounded">{s.vehicleName}</span></div>}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 overflow-hidden space-y-0.5">
                      {daySchedules.map(s => (
                        <div key={s.id} className={`text-[9px] p-1 rounded border leading-tight truncate ${s.vehicleId ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-indigo-50 text-indigo-700 border-indigo-100'}`}>
      ...
      {/* --- 月曆模式專用：當日資訊浮層 (Popover) --- */}
      {selectedDayInfo && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm md:absolute md:bg-white/10">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in duration-200">
            <div className="bg-indigo-600 p-4 text-white flex justify-between items-center">
              <h4 className="font-bold">{year}年{month + 1}月{selectedDayInfo.day}日 行程清單</h4>
              <button onClick={() => setSelectedDayInfo(null)}><i className="fas fa-times"></i></button>
            </div>
              ...
              {selectedDayInfo.schedules.map(s => (
                <div key={s.id} className="border-b border-slate-100 pb-3 last:border-0">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-indigo-600 font-bold text-sm">{s.startTime} - {s.endTime}</span>
                    <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded font-bold">{s.category}</span>
                  </div>
                  <div className="font-bold text-slate-800 text-sm mb-1">{s.userName} ➔ {s.projectName}</div>
                  <div className="text-xs text-slate-500 leading-relaxed mb-2 bg-slate-50 p-2 rounded-lg italic">
                    <i className="fas fa-pen-nib mr-1 text-slate-300"></i> {s.purpose}
                  </div>
                  <div className="grid grid-cols-1 gap-1.5">
                    {s.accompanimentIds?.length > 0 && (
                      <div className="text-[10px] text-pink-600 bg-pink-50 px-2 py-1 rounded flex items-center">
                        <i className="fas fa-user-friends mr-1.5"></i> 同行：{getCompanionNames(s.accompanimentIds)}
                      </div>
                    )}
                    {s.vehicleId && (
                      <div className="text-[10px] text-amber-700 bg-amber-50 px-2 py-1 rounded flex items-center">
                  ...
                  {(s.userId === currentUser.id || currentUser.role === UserRole.ADMIN) && (
                    <div className="flex gap-2 mt-3">
                      <button onClick={() => { onEdit(s); setSelectedDayInfo(null); }} className="flex-1 py-1.5 bg-slate-100 text-[11px] font-bold rounded-lg hover:bg-indigo-50 hover:text-indigo-600 transition">編輯</button>
                      <button onClick={() => { if(window.confirm('刪除行程？')) onDelete(s.id); setSelectedDayInfo(null); }} className="flex-1 py-1.5 bg-red-50 text-red-600 text-[11px] font-bold rounded-lg hover:bg-red-100 transition">刪除</button>
                    </div>
                  )}
                </div>
              ))}
            ...
            <div className="p-3 bg-slate-50 text-center">
              <button onClick={() => setSelectedDayInfo(null)} className="text-xs font-bold text-slate-400">關閉視窗</button>
            </div>
          </div>
        </div>
      )}
